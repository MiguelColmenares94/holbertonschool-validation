...
3. Run `make build` should generate both the static website with ./dist/ and the API application
...
6. Run `make clean` should delete all the generated files (logs, test reports, distribution directory, etc.) and call the make stop target as preliminary
7. Run `make run` should start the service in background so that http://localhost:9999/posts/welcome/ is reachable
